...
class MainActivity : AppCompatActivity() {
    ...
    fun populate_questions(): List<Map<Int, String>> {
        ...
        questions.add(mapOf(0 to "Ile jest kontynentów", 1 to "7", 2 to "8", 3 to "10", 4 to "-100", 5 to "1"))
        questions.add(mapOf(0 to "Ile jest województw w Polsce", 1 to "51", 2 to "34", 3 to "22", 4 to "16", 5 to "4"))
        ...
        questions.add(mapOf(0 to "Który kraj ma największą liczbę mieszkańców?", 1 to "Chiny", 2 to "Indie", 3 to "USA", 4 to "Rosja", 5 to "Chiny"))
    ...
    fun next_question(view: View?) {
        ...
        Log.i("MÓJ MESSAGE", "DObrze chłopaki robia")
        var questionObj = questions.get(question_num)
        val group: RadioGroup = findViewById(R.id.group)
        if(group.findViewById<RadioButton>(group.checkedRadioButtonId).text == questionObj.get(5)){
            correct_answers += 1
            Log.i("MÓJ MESSAGE", correct_answers.toString())
        }
        question_num += 1
        Log.i("MÓJ MESSAGE", "Pozdro z Legnicy")
        if(question_num >= 10){
            Log.i("LICZBA", question_num.toString())
//            ending_screen()
            return
        }
        ...
        question.text = questionObj.get(0).plus("?")
